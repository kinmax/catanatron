...
class SARSAPlayer(Player):
    def __init__(self, color, model_name):
        super(SARSAPlayer, self).__init__(color)
        self.model_path = f"./data/tables/sarsa-player/{model_name}/{model_name}.json"
        data = ""
        ...
        self.metrics_path = f"data/metrics/sarsa-player/{model_name}/benchmark_metrics.csv"
        self.known_states = 0
...
def main(experiment_name, gamma):
    ...
    models_folder = f"data/tables/sarsa-player/{model_name}/"
    if not os.path.isdir(models_folder):
        os.makedirs(models_folder)

    metrics_path = f"data/metrics/sarsa-player/{model_name}/"
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit="episodes"):
        ...
        best_action_int = epsilon_greedy_policy(env.playable_actions(), q_table[repr(current_state)], epsilon)
        ...
        while not done:
            ...
            new_best_action_int = epsilon_greedy_policy(env.playable_actions(), q_table[repr(new_state)], epsilon)

            if SHOW_PREVIEW and not episode % AGGREGATE_STATS_EVERY:
                env.render()

            q_table[repr(current_state)][best_action_int] = q_table[repr(current_state)][best_action_int] + alpha * (reward + (gamma * q_table[repr(new_state)][new_best_action_int]) - q_table[repr(current_state)][best_action_int])

            current_state = new_state
            best_action_int = new_best_action_int
            step += 1
